...
def enhancedFeatureExtractorDigit(datum):
    ...
    -Number of countiguous regions in the image 
    ...
    features["con0"]=1 if numOfCon ==0 else 0
    features["con1"]=1 if numOfCon ==1 else 0
    features["con2"]=1 if numOfCon ==2 else 0
    features["con3+"]=1 if numOfCon >=3 else 0
...
def enhancedPacmanFeatures(state, action):
    ...
    currentGhostStates=state.getGhostStates()
    currentPos=state.getPacmanPosition()

    currentGhostDistance=[[util.manhattanDistance(currentPos,ghostState.getPosition()),ghostState.scaredTimer]  for ghostState in currentGhostStates]
    closestScaredGhost = None
    if len(filter(lambda x: x[1]>0,currentGhostDistance)):
        closestScaredGhost=min(filter(lambda x: x[1]>0,currentGhostDistance))

    features["closestScaredGhost"]= 0 if closestScaredGhost == None else closestScaredGhost[0]**2

    newGameState=state.generatePacmanSuccessor(action)
    newPos=newGameState.getPacmanPosition()
    newCapsules=newGameState.getCapsules()
    newFood=newGameState.getFood()
    
    newGhostStates=newGameState.getGhostStates()
    closestGhostDistance=min([[util.manhattanDistance(newPos,ghostState.getPosition()),ghostState.scaredTimer]  for ghostState in newGhostStates])
    features["closestGhostDistance"]= 1.0/closestGhostDistance[0]**2 if closestGhostDistance[0] else 0

    features["newScore"]=newGameState.getScore()**2
    closestFood=0
    if len([util.manhattanDistance(newPos,food) for food in newFood.asList()]):
        closestFood=min([util.manhattanDistance(newPos,food) for food in newFood.asList()])
    features["closestFood"]=0 if closestFood == 0 else 1.0/closestFood**2
    return features
